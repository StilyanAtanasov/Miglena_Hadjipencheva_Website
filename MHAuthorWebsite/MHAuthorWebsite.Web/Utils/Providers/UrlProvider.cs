...
public class UrlProvider : IUrlProvider
{
    ...
    public string GetAdminOrderDetailsPageUrl(Guid orderId)
    => _linkGenerator.GetUriByAction(
            _httpContextAccessor.HttpContext!,
            action: "OrderDetails",
            controller: "AdminOrder",
            values: new { area = "Admin", orderId = orderId }) ?? string.Empty;
    ...
    public string GetMarketingUnsubscribeUrl(string email, string token)
    => _linkGenerator.GetUriByAction(
            _httpContextAccessor.HttpContext!,
            action: "UnsubscribeMarketing",
            controller: "Home",
            values: new { email, token }) ?? string.Empty;
}
